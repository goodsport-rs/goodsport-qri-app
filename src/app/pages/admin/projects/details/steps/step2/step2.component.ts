...
export class Step2Component implements OnInit, OnDestroy {
  ...
  getInterestedInvestors() {
    this.service.findInterestedInvestors(this.projectDetails.id).subscribe(
      (data: any) => {
  ...
  onClickIcon(id: string, type: string, index: number) {
    this.loaderId = index;
    this.statusLoadingSubject.next(true);
    this.service.approveRejectInvestor(id, type).subscribe(
      (data: any) => {
        this.statusLoadingSubject.next(false);
        this.sweetAlert.successMessage(`Investor ${type} successfully!`);
        this.allInvestors[index] = data;
      },
      (error) => {
        this.statusLoadingSubject.next(false);
        this.sweetAlert.errorMessage(error);
      ...
      .subscribe(
        (data: any) => {
          ...
          this.sweetAlert.successMessage(
            'Project FUNDING approved successfully!'
          );
          modal.close('');
